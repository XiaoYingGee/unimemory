# UniMemory P3 Benchmark Protocol v0.3.1

**作者**: 雪琪 ❄️ (起草) · 金瓶儿 🍾 (QA review) · 小白 🦊 (调度 sign-off)
**日期**: 2026-04-18
**状态**: v0.3.1 生效（v0.3 §4.3 + 范围脱敏 A/B 工具选项）
...
### 4.3 反向信号处理（v0.3 新增，2026-04-18 由 OPT-3 Step A open_domain 反向催生）

当 **Δ < 0**（新方案在某类目上比 baseline 差）且满足下列任一条件：

1. 三个 conv 的 delta **同向退步**（全负）
2. `|Δ题数| ≥ √n`

→ **原则上必须阻塞 merge**，直至：

- (a) 修复让 Δ ≥ 0（最低不退步），或
- (b) PM + QA 两人显式 ACK 接受退步并书面说明业务可承受理由

**禁止以「CI 重叠不算真信号」为由把反向同向归入 follow-up 队列**——CI 重叠只意味着统计学没法叫板，不代表问题不存在。反向同向是稳定副作用，必须直面。

对称原则：正向真信号需「三规则全过」才能锁结论；反向阻塞只需「同向 OR √n」其一，**对反向更严是因为副作用比错过收益代价更大**。

**范围脱敏（工具选项 A/B）**：「阻塞」可以在两种粒度上执行，由 PM+QA 根据本次特征选：

- **A. 整 PR 阻塞**（代码不 merge）——适用于：方案无 opt-in 独立验证价值、与后续修复耦合度高、merge 后产生多余 maintenance 负担
- **B. 代码 merge default OFF + opt-in flag**（默认行为翻转阻塞）——适用于：方案本身有 opt-in 使用场景、代码质量独立于默认体验、需要用户可手动验证

两者都满足「默认体验不静默退步」的红线。決策书应明写选 A 还是 B 及理由。
...
- v0.1 → v0.2（小白姐变更流程补丁 + 漂移防护附录）→ v0.3（§4.3 反向信号阻塞）
- 版本变更必须三人（雪琪 + 瓶儿 + 小白）ACK
- 文件路径：`docs/p3/benchmark-protocol.md`

### 变更记录

- **v0.3** (2026-04-18, 雪琪 commit) — 新增 §4.3 反向信号阻塞规则，由 OPT-3 Step A Wave 1 open_domain -10pp 弱反向案例催生。三人 ACK 完成。
- **v0.2** (2026-04-18, 雪琪 commit) — 协议变更流程 + 信息漂移防护附录
- **v0.1** (2026-04-18, 雪琪 commit) — 初版
