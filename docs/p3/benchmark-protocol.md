# UniMemory P3 Benchmark Protocol v0.3

**作者**: 雪琪 ❄️ (起草) · 金瓶儿 🍾 (QA review) · 小白 🦊 (调度 sign-off)
**日期**: 2026-04-18
**状态**: v0.3 生效（v0.2 + §4.3 反向信号阻塞）
...
**范围脱敏**（瓶儿拆解补丁）：本条只阻塞 **default 行为翻转**（默认启用该方案），**不阻塞代码合并进 main + opt-in flag 保留**。这让「代码质量」和「默认体验」在审核上解耦，避免为了防副作用雪藏真胜利代码。
